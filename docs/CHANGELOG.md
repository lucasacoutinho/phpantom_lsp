...
- **Per-subproject PHP version support.** Monorepos with subprojects targeting different PHP versions now get correct version-aware behaviour per file. Each subproject's PHP version is detected from its `composer.json` or `.phpantom.toml`, and stub filtering (`@removed`, `#[PhpStormStubsElementAvailable]`) is applied at query time using the file's subproject version rather than a single global version. Stub classes and functions are cached per version so that methods available in one subproject's PHP version are not incorrectly hidden in another. The negative class-resolution cache is also version-keyed to prevent cross-version poisoning. In single-project mode, behaviour is unchanged.
- **Background workspace indexing for find-references.** Symbol maps for all user PHP files are now built in the background during server initialization, so the first find-references request no longer triggers an expensive full-workspace scan. A tri-state atomic flag (`NOT_STARTED` / `IN_PROGRESS` / `COMPLETE`) prevents duplicate scans when find-references arrives before background indexing finishes.

### Changed

- **Find-references excludes unresolvable subjects.** When a member-access subject (e.g. `$unknown->save()`) cannot be resolved and a known class hierarchy exists, the reference is now excluded rather than conservatively included. Unresolvable subjects are more likely false positives than true matches when the target hierarchy is known.
